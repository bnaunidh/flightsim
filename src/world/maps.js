 ...
 * Town 6.81 -> 10.95 m, Airfield Perimeter 8.96 -> 11.75 m.
 * Bent and regraded Town roads (8.50% max authored grade), varied Delta
 * banks, broadened Flat Rock. Nine lighthouse origins moved onto land.
 * IDs, existing island indices, mission names and runway positions retained.
 * 32 scenery builds and runway profiles checked; final suite 146/146.
 * School-device frame rates and real offline installation were not checked. */
/**
 * Maps.
 *
...
export const MAPS = [
  {
    ...
    islands: [
      /*
       * Peak 40 on 'plains', not 8, and four hill groups rather than two.
       *
       * The delivered fix for the bulldozed cross was to take the island
       * away: island 0 went from 'hills' at 300 to 'plains' at 8, which is
       * a green plate 2.4 km across, and the 42.8 m standard deviation it
       * reported came entirely from two knolls sitting on top of it. The
       * cross was gone and so was Kestrel.
       *
       * Measured on the same 128x128 grid the delivery used: at 'plains' 40
       * with hills in all four quadrants the corridor still cuts 0.00% of
       * the land and 0.4 m at the worst point — nothing — while the
       * standard deviation goes 42.8 -> 47.6 and the mean land height
       * 40 -> 57 m. Going further costs real ground: 'plains' 70 cuts
       * 0.48%, 'hills' 70 cuts 3.6% and 58 m, 'hills' 150 cuts 13.4%.
       */
      { name: 'Kestrel Island', cx: 0, cz: 0, radius: 2450, peak: 40, seed: 3, profile: 'plains' },
      ...
      // Hills sit clear of both approach axes; the airfield stays at its mission coordinates.
      { name: 'North Kestrel Hills', cx: -1200, cz: -1450, radius: 680, peak: 185, seed: 37, profile: 'hills' },
      { name: 'South Kestrel Hills', cx: 1350, cz: 1400, radius: 640, peak: 160, seed: 43, profile: 'hills' },
      // The other two quadrants, so the island has something in every
      // direction rather than high ground on one diagonal and nothing on
      // the other. Both clear of the two approach axes, same as above.
      { name: 'East Kestrel Hills', cx: 1300, cz: -1500, radius: 620, peak: 170, seed: 53, profile: 'hills' },
      { name: 'West Kestrel Hills', cx: -1350, cz: 1450, radius: 600, peak: 150, seed: 59, profile: 'hills' },
    ],
    chunks: [
  ...
  /* town — car-map-2-town.js, 450 m strip sited on Fenwick Island, 1.4 m of levelling */
  {
    ...
    blurb: 'A real town: winding streets, a square in the middle and a ring road all the way round it. Every corner is a choice — the short way through the middle, or the long way round on the ring. Deliveries are waiting all over town.',
    difficulty: 1,
    difficultyLabel: 'Gentle',
    game: 'car',
    games: ['car'],
      ...
      roads: [
        ...
        },
        {
          ...
          path: [
            ...
            [-451.13, 865.32, 41.19],
            [-427.6, 959.68, 40.44],
            [-404.08, 1055.86, 40.83],
            [-383.74, 1154.03, 39.19],
            [-375.66, 1203.89, 39.99],
            [-369.35, 1254.27, 41.92],
            [-362.86, 1356.58, 45.76],
            [-365.15, 1460.84, 45.84],
          ],
        },
        {
          halfWidth: 8,
          blend: 22,
          path: [[365.15, 500, 47.16], [369.35, 719.04, 40.65], [404.08, 934.68, 37.28], [451.13, 1144.14, 40.65], [485.86, 1345.73, 45.22], [490.05, 1539.16, 51.55]],
        },
        {
          halfWidth: 11,
          blend: 26,
        ...
        },
        {
          ...
          path: [
            ...
            [590.7, 933.72, 45.08],
            [614.22, 1039.2, 51.05],
            [637.75, 1142.92, 53.15],
            [658.09, 1244.69, 51.79],
            [666.17, 1294.83, 52.24],
            [672.48, 1344.46, 53.69],
            [678.97, 1442.21, 55.33],
            [676.67, 1538.07, 52.83],
          ],
        },
        {
          halfWidth: 11,
          ...
          path: [
            [676.67, 1538.07, 52.83],
            [585.51, 1540.71, 53.95],
            [490.05, 1539.16, 51.55],
            [389.46, 1533.56, 46.23],
            [283.84, 1524.5, 44.41],
            [174.24, 1512.92, 42.39],
            [62.45, 1500, 42.66],
            [-49.34, 1487.08, 43.99],
            [-158.94, 1475.5, 45.36],
            [-264.56, 1466.44, 45.37],
            [-365.15, 1460.84, 45.84],
            [-460.61, 1459.29, 48.94],
            [-551.78, 1461.93, 48.69],
          ],
        },
        {
          halfWidth: 11,
          ...
          path: [
            [-551.78, 1461.93, 48.69],
            [-549.48, 1357.79, 51.76],
            [-555.97, 1255.54, 47.38],
            [-562.28, 1205.17, 46.6],
            [-570.36, 1155.31, 47.41],
            [-590.7, 1057.08, 46.5],
            [-614.22, 960.8, 49.27],
            [-637.75, 866.28, 52.13],
            [-658.09, 773.28, 52.84],
        ...
        },
        {
          ...
          path: [
            ...
            [0, 1000, 45.15],
            [111.79, 1013.3, 41.93],
            [221.39, 1025.23, 39.19],
            [327.01, 1034.56, 38.81],
            [427.6, 1040.32, 38.93],
            [523.06, 1041.93, 43.3],
            [614.22, 1039.2, 51.05],
          ],
        },
        {
          ...
          path: [
            [-562.28, 1205.17, 46.6],
            [-471.11, 1202.06, 44.03],
            [-375.66, 1203.89, 39.99],
            [-275.07, 1210.48, 40.79],
            [-169.45, 1221.15, 41.7],
            [-59.85, 1234.79, 42.38],
            [51.95, 1250, 42.18],
            [163.74, 1265.21, 42.02],
            [273.34, 1278.85, 43.89],
            [378.96, 1289.52, 42.41],
            [479.55, 1296.11, 44.11],
            [575.01, 1297.94, 45.3],
            [666.17, 1294.83, 52.24],
          ],
        },
        { halfWidth: 50, blend: 70, path: [[-33.64, 995.95, 45.45], [33.64, 1004.05, 44.84]] },
      ],
    },
    palette: {
      grass: [1.02, 1, 0.82],
  ...
  /* ravencrag — heli-map-rescue.js, 450 m strip sited on Sentinel Knoll, 1.9 m of levelling */
  {
    ...
    islands: [
      { name: 'Sentinel Knoll', cx: 0, cz: 0, radius: 700, peak: 110, seed: 701, profile: 'plains' },
      {
        name: 'Corrie Floor',
        cx: 0,
        cz: -5200,
  ...
  /* meridian — meridian-city.heli-map.js, 450 m strip sited on Meridian Island, 1.4 m of levelling */
  {
    ...
    islands: [
      { name: 'Meridian Island', cx: 0, cz: 0, radius: 2300, peak: 90, seed: 601, profile: 'plains' },
      {
        name: 'Signal Point',
        cx: 4450,
        cz: -2200,
